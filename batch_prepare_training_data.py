#!/usr/bin/env python3
"""
从原始场景目录批量生成 RoomFormer 训练目录与 COCO 标注。

原始场景（每套一个子目录）建议结构：
  {scene_id}/
    Datas/ViewData.txt
    point_clouds_gen/*.ply          # 或 --ply-subdir frontend
    images/*.jpg                    # 全景
    images_depth/*_depth.png        # 深度（可选，仅拷贝）
    images_depth/*_confidence.png
    images_depth/depthConfig.json
    floorplan_gt.json               # 可选：本脚本识别的 GT，见下文

输出（--out）：
  {out}/
    train/{scene_id}/
      bev_geometric.png
      bev_meta.json                 # 训练反投影用
      bev_geometric.json / _coverage.npz  # BEV 元数据（可选）
      viewData.txt                  # 从 Datas/ViewData.txt 复制
      panoramas/*.jpg               # 从 images/ 复制
      depths/*_depth.png 等         # 从 images_depth/ 复制（可选）
    val/{scene_id}/...
    annotations/train.json
    annotations/val.json

floorplan_gt.json（可选，与 BEV 像素坐标一致，宽=x、高=z 栅格与 bev_geometric.png 相同）：
  {
    "annotations": [
      {
        "category_id": 0,
  ...
  door/window 若为多边形，可与官方数据一样后续再扩展；当前按房间多边形训练。

若 floorplan_gt.json 不存在，脚本会自动回退到 ViewData.txt：
  - 按 test_pose.py / datasets.dataLoader.RoomFormerV1Dataset.__getitem__ 的转换逻辑
  - 将房间墙体点位转换到 BEV 像素，生成 COCO segmentation
  - semantic 模式下额外生成 door/window 线段标注（2 点）

用法示例：
  python batch_prepare_training_data.py \\
      --scenes-root /data/raw_scenes \\
      --out /data/dataset_v1 \\
      --val-ratio 0.1

  # 同时批量从深度生成点云再算 BEV
  python batch_prepare_training_data.py --scenes-root /data/raw --out /data/out --gen-ply

  # 使用符号链接节省空间
  python batch_prepare_training_data.py --scenes-root /data/raw --out /data/out --symlink

  # 训练时：--dataset_root /data/dataset_v1
"""
